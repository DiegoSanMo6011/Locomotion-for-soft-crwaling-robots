...
/*---------- PARÁMETROS DEL CONTROLADOR PI (ACTUALIZADOS para NEGATIVO) ----------*/
// Kp: -200.33, Ki: -1670.3
// NOTA: Estas ganancias son NEGATIVAS para compensar la ganancia de planta negativa.
const float KP_CONTROLLER = -200.33f;
const float KI_CONTROLLER = -1670.3f;

// Tiempo de muestreo (10 ms = 0.01 s).
const float TS_CONTROLLER = 0.010f; 

// REFERENCIA DENTRO DEL RANGO LINEAL (-32 kPa a -34 kPa)
const float pressure_reference = -10.0f; // KPa a la que queremos llegar

/*---------- VARIABLES DE CONTROL ----------*/
float integral_sum = 0.0f; 

/*---------- MODELO DEL SENSOR DE PRESIÓN ----------*/
// (Ajustar a su sensor, se mantienen los valores del código original)
const float V_OFFSET  = 2.5f;   // Voltaje a 0 kPa
const float V_PER_kPa = 0.02f;  // Volt/ kPa

/*---------- PARÁMETROS DEL EXPERIMENTO ----------*/
const int PWM_BASE = 0; // PWM en reposo
...
/*---------- MÁQUINA DE ESTADOS (Se mantiene) ----------*/
enum State {
...
/*---------- ALGORITMO DE CONTROL PI EN TIEMPO DISCRETO (Ajuste Anti-Windup) ----------*/
void runPIControl(float currentPressure) {
  ...
  // --- Anti-windup Básico ---
  // Si la salida predicha está saturada y el error sigue empujando la salida
  // MÁS ALLÁ del límite, revertimos la integración. 
  // Nota: error * KP_CONTROLLER tiene el signo correcto del esfuerzo
  float predicted_pwm = (KP_CONTROLLER * error) + (KI_CONTROLLER * integral_sum);
  if ((predicted_pwm > PWM_MAX && error * KP_CONTROLLER > 0) || 
      (predicted_pwm < 0 && error * KP_CONTROLLER < 0)) {
